...
RSpec.describe PostSerializer do
  ...
  describe "#my_retorts" do
    it "should add my retorts" do
      ...
      expect(post_serializer[:my_retorts].pluck("emoji")).to match_array(["heart", "+1"])
  ...
  describe "#can_retort" do
    it "should be true for normal user" do
      expect(PostSerializer.new(post, scope: Guardian.new(user1)).as_json[:post][:can_retort]).to eq(true)
    end
